...
import static com.twosigma.beakerx.AutotranslationService.BEAKERX;
...
public class KernelTest implements KernelFunctionality {
  ...
  public KernelTest(ResultSender resultSender, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
    this("KernelTestId1", new BeakerXCommRepositoryMock(), messageCreatorService, magicCommandConfiguration);
    this.executionResultSender = resultSender;
  }

  public KernelTest(MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
    this("KernelTestId1", new BeakerXCommRepositoryMock(), messageCreatorService, magicCommandConfiguration);
  }

  public KernelTest(CommRepository commRepository, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
    this("KernelTestId1", commRepository, messageCreatorService, magicCommandConfiguration);
  }

  public KernelTest(String id, CommRepository commRepository, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
    this.id = id;
    ...
    this.executionResultSender = new ExecutionResultSender(this, messageCreatorService);
    EvaluationObject value = new SimpleEvaluationObjectMock("ok", new SeoConfigurationFactoryMock(this, commMsg(), messageCreatorService, this.magicCommandConfiguration));
    InternalVariable.setValue(value);
    KernelManager.register(this);

  }

  public KernelTest(String id, Evaluator evaluator, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
    this.id = id;
  ...
  public void sendBusyMessage(Message message) {
    Message busyMessage = messageCreatorService.createBusyMessage(message);
    publish(Collections.singletonList(busyMessage));
  }

  @Override
  public void sendIdleMessage(Message message) {
    Message idleMessage = messageCreatorService.createIdleMessage(message);
  ...
  public FileServiceMock getFileService() {
    return (FileServiceMock) magicCommandConfiguration.getFileService();
  }
  ...
  public static class SeoConfigurationFactoryMock implements ConfigurationFactory {
    ...
    public SeoConfigurationFactoryMock(KernelFunctionality kernel, Message message, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
      this.kernel = kernel;
    ...
    public SeoConfigurationFactoryMock(MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
      this(new KernelTest(messageCreatorService, magicCommandConfiguration), messageCreatorService, magicCommandConfiguration);
    }

    public SeoConfigurationFactoryMock(Message message, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
      this(new KernelTest(messageCreatorService, magicCommandConfiguration), message, messageCreatorService, magicCommandConfiguration);
    }

    public SeoConfigurationFactoryMock(int executionCount, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
      this(messageCreatorService, magicCommandConfiguration);
      this.executionCount = executionCount;
    }

    public SeoConfigurationFactoryMock(KernelTest kernel, MessageCreatorService messageCreatorService, MagicCommandConfiguration magicCommandConfiguration) {
      this(kernel, commMsg(), messageCreatorService, magicCommandConfiguration);
    }

    @Override
    public Configuration create(EvaluationObject seo) {
      BeakerOutputHandler stdout = new SimpleOutputHandler(false, kernel.getExecutionResultSender(), seo);
      BeakerOutputHandler stderr = new SimpleOutputHandler(true, kernel.getExecutionResultSender(), seo);
  ...
  public static class ExecutionResultSenderMock implements ResultSender {
    ...
    private List<EvaluationObject> objectList = new LinkedList<>();
    ...
    public List<EvaluationObject> getObjectList() {
      return objectList;
    }
  }
