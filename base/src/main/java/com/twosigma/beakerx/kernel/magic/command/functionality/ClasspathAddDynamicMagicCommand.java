...
public class ClasspathAddDynamicMagicCommand extends ClasspathMagicCommand {
  ...
  public MagicCommandOutcomeItem execute(MagicCommandExecutionParam param) {
    ...
    if (split.length < 4) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, WRONG_FORMAT_MSG + CLASSPATH_ADD_DYNAMIC, MessageCreator.get());
    }

    String codeToExecute = command.substring(command.indexOf(DYNAMIC) + DYNAMIC.length()).trim();
    EvaluationObject seo = objectFactory.createSeo(codeToExecute, kernel, param.getCode().getMessage(), param.getExecutionCount());
    TryResult either = kernel.executeCode(codeToExecute, seo);
    if (either.isResult()) {
      try {
        return addJars(either.result());
      } catch (Exception e) {
        return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "There occurs problem during execution of " + CLASSPATH_ADD_DYNAMIC + " : " + e.getMessage(), MessageCreator.get());
      }
    } else {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "There occurs problem during execution of " + CLASSPATH_ADD_DYNAMIC + " : " + either.error(), MessageCreator.get());
    }
  }
