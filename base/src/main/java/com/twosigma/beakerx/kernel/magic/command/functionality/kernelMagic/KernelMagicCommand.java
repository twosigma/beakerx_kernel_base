...
public class KernelMagicCommand implements MagicCommandFunctionality {
  ...
  public MagicCommandOutcomeItem execute(MagicCommandExecutionParam param) {
    ...
    if (!validateCommandFormat(param)) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, getFormatErrorMessage(), MessageCreator.get());
    }
    String kernelName = getKernelName(param);
    try {
      pep = kernel.getPythonEntryPoint(kernelName);
    } catch (NoSuchKernelException e) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, String.format("Kernel %s is not available!", e.getMessage()), MessageCreator.get());
    }
