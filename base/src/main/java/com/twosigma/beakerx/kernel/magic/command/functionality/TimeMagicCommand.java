...
public abstract class TimeMagicCommand implements MagicCommandFunctionality {
  ...
  public TimeMagicCommand(KernelFunctionality kernel, EvaluationObjectFactory objectFactory) {
    this.kernel = kernel;
  ...
  public MagicCommandOutput time(String codeToExecute, Message message, int executionCount, boolean showResult) {
    ...
    EvaluationObject simpleEvaluationObject = objectFactory.createSeo(codeToExecute, kernel, message, executionCount);
    if (!showResult) {
    ...
    try {
      ...
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "There occurs problem during measuring time for your statement.", MessageCreator.get());
    }
  }
  ...
  protected MagicCommandOutput timeIt(TimeItOption timeItOption, String codeToExecute, Message message, int executionCount, boolean showResult) {
    ...
    if (timeItOption.getNumber() < 0) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "Number of execution must be bigger then 0", MessageCreator.get());
    }
    ...
    if (timeItOption.getRepeat() == 0) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "Repeat value must be bigger then 0", MessageCreator.get());
    }

    EvaluationObject seo = objectFactory.createSeo(codeToExecute, kernel, message, executionCount);
    seo.noResult();
    ...
    try {
      ...
      if (either.isError()) {
        return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "Please correct your statement", MessageCreator.get());
      }
      ...
      IntStream.range(0, timeItOption.getRepeat()).forEach(repeatIter -> {
        IntStream.range(0, number).forEach(numberIter -> {
          EvaluationObject seo2 = objectFactory.createSeo(codeToExecute, kernel, message, executionCount);
          seo2.noResult();
      ...
      if (isReady.get()) {
        ...
        return new MagicCommandOutput(MagicCommandOutput.Status.OK, output, MessageCreator.get());
      }
    } catch (InterruptedException | ExecutionException e) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "There occurs problem with " + e.getMessage(), MessageCreator.get());
    }
    return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, "There occurs problem with timeIt operations", MessageCreator.get());
  }
  ...
  private int getBestNumber(String codeToExecute, boolean showResult, Message message) {
    for (int value = 0; value < 10; ) {
      ...
      IntStream.range(0, numberOfExecution.intValue()).forEach(indexOfExecution -> {
        EvaluationObject simpleEvaluationObject = objectFactory.createSeo(
