...
public class ClasspathResetMagicCommand implements MagicCommandFunctionality {
  ...
  public MagicCommandOutcomeItem execute(MagicCommandExecutionParam param) {
    ...
    if (split.length != 2) {
      return new MagicCommandOutput(Status.ERROR, WRONG_FORMAT_MSG + CLASSPATH_RESET, MessageCreator.get());
    }
    ClasspathAddMvnMagicCommand mvnMagicCommand = getClasspathAddMvnMagicCommand();
    ...
    try {
      ...
      return new MagicCommandOutput(Status.ERROR, e.getMessage(), MessageCreator.get());
    }
    return new MagicCommandOutput(Status.OK, "Reset done, please restart the kernel.", MessageCreator.get());
  }
