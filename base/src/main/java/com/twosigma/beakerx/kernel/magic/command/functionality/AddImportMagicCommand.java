...
import static com.twosigma.beakerx.kernel.magic.command.functionality.AddImportMagicCommandInfo.IMPORT;
...
public class AddImportMagicCommand implements MagicCommandFunctionality {
  ...
  public AddImportMagicCommand(KernelFunctionality kernel, MessageCreatorService messageCreatorService) {
    this.kernel = kernel;
  ...
  public MagicCommandOutcomeItem execute(MagicCommandExecutionParam param) {
    ...
    if (parts.length != 2) {
      return new MagicCommandOutput(ERROR, WRONG_FORMAT_MSG + IMPORT, messageCreatorService);
    }
    ...
    if (AddImportStatus.ERROR.equals(status)) {
      return new MagicCommandOutput(ERROR, "Could not import " + parts[1], messageCreatorService);
    }
    return new MagicCommandOutput(OK, messageCreatorService);
  }
}
