...
public class LoadMagicMagicCommand implements MagicCommandFunctionality, LoadMagicMagicCommandInfo {
  ...
  public MagicCommandOutcomeItem execute(MagicCommandExecutionParam param) {
    ...
    if (split.length != 2) {
      return new MagicCommandOutput(ERROR, WRONG_FORMAT_MSG + LOAD_MAGIC, messageCreatorService);
    }
  ...
  public MagicCommandOutcomeItem load(String clazzName) {
    try {
      ...
      if (instance instanceof MagicCommandFunctionality) {
        ...
        return new MagicCommandOutput(OK, "Magic command " + commandFunctionality.getMagicCommandName() + " was successfully added.", messageCreatorService);
      } else {
        return new MagicCommandOutput(ERROR, "Magic command have to implement " + MagicCommandFunctionality.class + " interface.", messageCreatorService);
      }
    } catch (Exception e) {
      return new MagicCommandOutput(ERROR, e.toString(), messageCreatorService);
    }
  }
}
