...
public class AddStaticImportMagicCommand implements MagicCommandFunctionality {
  ...
  public MagicCommandOutcomeItem execute(MagicCommandExecutionParam param) {
    ...
    if (parts.length != 3) {
      return new MagicCommandOutput(MagicCommandOutput.Status.ERROR, WRONG_FORMAT_MSG, MessageCreator.get());
    }
    AddImportStatus status = this.kernel.addImport(new ImportPath(parts[1] + " " + parts[2]));
    if (AddImportStatus.ERROR.equals(status)) {
      return new MagicCommandOutput(ERROR, "Could not import static " + parts[2], MessageCreator.get());
    }
