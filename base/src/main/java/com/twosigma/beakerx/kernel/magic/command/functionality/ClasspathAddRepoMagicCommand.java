...
public class ClasspathAddRepoMagicCommand implements MagicCommandFunctionality {
  ...
  private MagicCommandOutcomeItem handleNewRepo(String[] split) {
    if (split.length != 5) {
      return new MagicCommandOutput(Status.ERROR, WRONG_FORMAT_MSG + CLASSPATH_CONFIG_RESOLVER, MessageCreator.get());
    }
    String repoName = split[3];
    String urlName = split[4];
    ClasspathAddMvnMagicCommand command = getClasspathAddMvnMagicCommand();
    String result = command.addRepo(repoName, urlName);
  ...
  private MagicCommandOutcomeItem handleMvnLocal() {
    ...
    if (Files.exists(Paths.get(localRepo))) {
      ClasspathAddMvnMagicCommand command = getClasspathAddMvnMagicCommand();
      try {
        String result = command.addRepo(MVN_LOCAL, new File(localRepo).toURI().toURL().toString());
        return createResult(result);
      } catch (MalformedURLException e) {
        return new MagicCommandOutput(Status.ERROR, e.toString(), MessageCreator.get());
      }
    }
    return new MagicCommandOutput(Status.OK, String.format("Warning: directory %s not found", localRepo), MessageCreator.get());
  }

  private ClasspathAddMvnMagicCommand getClasspathAddMvnMagicCommand() {
    MagicCommandFunctionality mvnMagicCommand = kernel.magicCommandConfiguration().getClasspathAddMvnMagicCommand(kernel);
    return (ClasspathAddMvnMagicCommand) mvnMagicCommand;
  }

  private MagicCommandOutcomeItem createResult(String result) {
    if (result.isEmpty()) {
      return new MagicCommandOutput(Status.OK, MessageCreator.get());
    } else {
      return new MagicCommandOutput(Status.OK, ADDED_NEW_REPO + result, MessageCreator.get());
    }
  }
